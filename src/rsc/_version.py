...
def _sdk_headers(product: str = None, product_version: str = None) -> dict:
    ...
    ``product``, it is reflected verbatim in ``Sdk-Language``, its version in
    ``Sdk-Version``, and it leads the ``User-Agent`` — letting RSC's existing
    api-server metrics pipeline identify the traffic. The consumer owns the
    exact ``Sdk-Language`` value; ``rsc-client`` adds no language suffix.
    """
    if not product:
